...
broker_address = "192.168.1.80" #"172.20.10.3" # IP del broker MQTT
broker_port = 1883
...
current_latitude = 40.8650612943149
current_longitude = 14.2689142049037

def get_gps_data():
    global current_latitude, current_longitude
    
    # Incrementa le coordinate (puoi personalizzare l'incremento)
    current_latitude += random.uniform(-0.0001, 0.0001)  # Incremento casuale
    current_longitude += random.uniform(-0.0001, 0.0001)  # Incremento casuale
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print("Connesso con successo al broker MQTT, topic: router/gps")
    else:
